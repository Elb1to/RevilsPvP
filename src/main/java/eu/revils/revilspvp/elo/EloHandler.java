package eu.revils.revilspvp.elo;
...
import eu.revils.revilspvp.elo.listener.EloLoadListener;
import eu.revils.revilspvp.elo.listener.EloUpdateListener;
import eu.revils.revilspvp.elo.repository.EloRepository;
import eu.revils.revilspvp.elo.repository.MongoEloRepository;
...
public final class EloHandler {
    ...
    public EloHandler() {
        Bukkit.getPluginManager().registerEvents(new EloLoadListener(this), RevilsPvP.getInstance());
    ...
    public void resetElo(final UUID player) {
        Bukkit.getLogger().info("Resetting elo of " + RevilsPvP.getInstance().getUuidCache().name(player) + ".");
        Bukkit.getScheduler().runTaskAsynchronously(RevilsPvP.getInstance(), () -> {
