package eu.revils.revilspvp.kittype;
...
public final class KitType {
    ...
    static {
        ...
        collection.find().iterator().forEachRemaining(doc -> {
            allTypes.add(RevilsPvP.plainGson.fromJson(doc.toJson(), KitType.class));
        });
    ...
    public void saveAsync() {
        Bukkit.getScheduler().runTaskAsynchronously(RevilsPvP.getInstance(), () -> {
            MongoCollection<Document> collection = MongoUtils.getCollection(MONGO_COLLECTION_NAME);
            Document kitTypeDoc = Document.parse(RevilsPvP.plainGson.toJson(this));
