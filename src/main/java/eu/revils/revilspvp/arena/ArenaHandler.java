...
import eu.revils.revilspvp.arena.event.ArenaAllocatedEvent;
import eu.revils.revilspvp.arena.event.ArenaReleasedEvent;
import eu.revils.revilspvp.arena.listener.ArenaItemResetListener;
...
public final class ArenaHandler {
    ...
    public ArenaHandler() {
        Bukkit.getPluginManager().registerEvents(new ArenaItemResetListener(), RevilsPvP.getInstance());
        ...
        try {
            ...
            if (arenaInstancesFile.exists()) {
                try (Reader arenaInstancesReader = Files.newReader(arenaInstancesFile, Charsets.UTF_8)) {
                    ...
                    List<Arena> arenaList = RevilsPvP.getGson().fromJson(arenaInstancesReader, arenaListType);
            ...
            if (schematicsFile.exists()) {
                try (Reader schematicsFileReader = Files.newReader(schematicsFile, Charsets.UTF_8)) {
                    ...
                    List<ArenaSchematic> schematicList = RevilsPvP.getGson().fromJson(schematicsFileReader, schematicListType);
    ...
    public void saveSchematics() throws IOException {
        Files.write(
            RevilsPvP.getGson().toJson(schematics.values()),
    ...
    public void saveArenas() throws IOException {
        ...
        Files.write(
            RevilsPvP.getGson().toJson(allArenas),
