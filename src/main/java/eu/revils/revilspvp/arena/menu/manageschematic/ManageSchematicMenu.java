package eu.revils.revilspvp.arena.menu.manageschematic;
...
import eu.revils.revilspvp.util.menu.BooleanTraitButton;
import eu.revils.revilspvp.util.menu.IntegerTraitButton;
...
public final class ManageSchematicMenu extends Menu {
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        if (RevilsPvP.getInstance().getArenaHandler().getGrid().isBusy()) {
        ...
        Consumer<ArenaSchematic> save = schematic -> {
            try {
                RevilsPvP.getInstance().getArenaHandler().saveSchematics();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        };
