package eu.revils.revilspvp.pvpclasses;

import eu.revils.revilspvp.RevilsPvP;
import eu.revils.revilspvp.pvpclasses.event.BardRestoreEvent;
...
public class PvPClassHandler extends BukkitRunnable implements Listener {
    ...
    public PvPClassHandler() {
        ...
        for (PvPClass pvpClass : pvpClasses) {
            RevilsPvP.getInstance().getServer().getPluginManager().registerEvents(pvpClass, RevilsPvP.getInstance());
        }

        RevilsPvP.getInstance().getServer().getScheduler().runTaskTimer(RevilsPvP.getInstance(), this, 2L, 2L);
        RevilsPvP.getInstance().getServer().getPluginManager().registerEvents(this, RevilsPvP.getInstance());
    }
