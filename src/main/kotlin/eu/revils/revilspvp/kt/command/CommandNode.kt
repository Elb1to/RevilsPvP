...
import eu.revils.revilspvp.kt.command.data.argument.Arguments
    ...
    operator fun invoke(sender: CommandSender, arguments: Arguments): Boolean {
        ...
        for (data in this.parameters) {
            if (data is FlagData) {
                ...
                var type: ParameterType<*>? = RevilsPvP.getInstance().commandHandler.getParameterType(data.type)
        ...
        try {
            ...
            if (!async && logToConsole && stop - start >= executionThreshold) {
                RevilsPvP.getInstance().logger.warning("Command '/" + getFullLabel() + "' took " + (stop - start) + "ms!")
            }
