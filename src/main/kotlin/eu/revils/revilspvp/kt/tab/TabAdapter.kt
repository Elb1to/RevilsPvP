...
class TabAdapter : PacketAdapter(RevilsPvP.getInstance(), PacketType.Play.Server.PLAYER_INFO, PacketType.Play.Server.NAMED_ENTITY_SPAWN) {

    override fun onPacketSending(event: PacketEvent) {
        if (RevilsPvP.getInstance().tabEngine.layoutProvider == null || !this.shouldForbid(event.player)) {
            return
        }
    ...
    private fun shouldForbid(player: Player): Boolean {
        ...
        val playerTab = RevilsPvP.getInstance().tabEngine.tabs[playerName]
