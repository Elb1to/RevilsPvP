...
class TabEngine {
    ...
    fun load() {
        ...
        TabThread().start()
        RevilsPvP.getInstance().server.pluginManager.registerEvents(TabListeners(), RevilsPvP.getInstance())
    }
    ...
    private fun fetchSkin(): PropertyMap? {
        val propertyMap = RevilsPvP.getInstance().redis.runBackboneRedisCommand { redis ->
