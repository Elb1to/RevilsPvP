package eu.revils.revilspvp.kt.redis
...
class Redis : Closeable {
    ...
    fun load(localCredentials: RedisCredentials, backboneCredentials: RedisCredentials) {
        try {
            ...
            RevilsPvP.getInstance().logger.warning("Couldn't connect to a Redis instance at ${localCredentials.host}:${localCredentials.port}")
            e.printStackTrace()
        }
        ...
        try {
            ...
            RevilsPvP.getInstance().logger.warning("Couldn't connect to a Backbone Redis instance at ${backboneCredentials.host}:${backboneCredentials.port}")
            e.printStackTrace()
        }
    }
