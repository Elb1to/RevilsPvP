...
class Scoreboard(private val player: Player) {
    ...
    init {
        val board = RevilsPvP.getInstance().server.scoreboardManager.newScoreboard
    ...
    fun update() {
        val configuration = RevilsPvP.getInstance().scoreboardEngine.configuration!!
    ...
    private fun separate(line: String, usedBaseScores: MutableCollection<String>): Array<String> {
        ...
        when (this.separateScores.size) {
            ...
            3 -> {
                ...
                RevilsPvP.getInstance().logger.warning("Failed to separate scoreboard line. Input: $line")
            }
        }
