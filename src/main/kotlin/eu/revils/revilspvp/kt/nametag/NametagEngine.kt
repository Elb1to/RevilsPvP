...
import eu.revils.revilspvp.kt.scoreboard.ScoreboardTeamPacketMod
...
class NametagEngine {
    ...
    fun load() {
        if (RevilsPvP.getInstance().config.getBoolean("disableNametags", false)) {
            RevilsPvP.getInstance().logger.info("Nametags are disabled by config")
            return
        }

        isNametagRestrictionEnabled = RevilsPvP.getInstance().config.getBoolean("NametagPacketRestriction.Enabled", false)
        nametagRestrictBypass = RevilsPvP.getInstance().config.getString("NametagPacketRestriction.BypassPrefix", "").replace("&", "§")

        NametagThread().start()
        RevilsPvP.getInstance().server.pluginManager.registerEvents(NametagListener(), RevilsPvP.getInstance())
    ...
    fun reloadOthersFor(refreshFor: Player) {
        for (toRefresh in RevilsPvP.getInstance().server.onlinePlayers) {
    ...
    internal fun applyUpdate(nametagUpdate: NametagUpdate) {
        val toRefreshPlayer = RevilsPvP.getInstance().server.getPlayerExact(nametagUpdate.toRefresh)?: return

        if (nametagUpdate.refreshFor == null) {
            for (refreshFor in RevilsPvP.getInstance().server.onlinePlayers) {
                reloadPlayerInternal(toRefreshPlayer, refreshFor)
            }
        } else {
            val refreshForPlayer = RevilsPvP.getInstance().server.getPlayerExact(nametagUpdate.refreshFor)
    ...
    internal fun getOrCreate(prefix: String, suffix: String): NametagInfo {
        ...
        for (player in RevilsPvP.getInstance().server.onlinePlayers) {
